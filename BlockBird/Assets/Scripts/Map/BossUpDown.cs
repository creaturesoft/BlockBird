...
public class BossUpDown : MapBase
{
        ...
        while (true)
        {
            ...
            for (int y = -Steps; y <= Steps; y++)
            {
                ...
                else
                {
                    ...
                    {
                        isSetSpeed = true;
                        GameManager.Instance.Character.Speed = GameManager.Instance.Character.IsMeleeAttack ? 2.0f : 3.0f;
                    }
                }
                ...
                if (blockPosition == y)
                {
                    if (y == Steps || y == -Steps)
                    {
                        ...
                        blockList.Add(Instantiate(block1Prefab, new Vector3(0, BlockGap * y, 0), Quaternion.identity)
                               .Init(currentDepth.transform, Random.Range(lifeWeight * 1.5f, lifeWeight * 2.5f), 5, true));
                    }
                    else
                    {
                        blockList.Add(Instantiate(block1Prefab, new Vector3(0, BlockGap * y, 0), Quaternion.identity)
                               .Init(currentDepth.transform, Random.Range(lifeWeight/2, lifeWeight), 5, true));
