...
public class BossDiamond : MapBase
{
    ...
    IEnumerator SpawnBlock()
    {
        ...
        while (true)
        {
            ...
            for (int y = -Steps; y <= Steps; y++)
            {
                ...
                if (totalDepth == 50 || totalDepth == 56)
                {
                    if (y == 0)
                    {
                ...
                else if (totalDepth == 51 || totalDepth == 55)
                {
                    if (y == -1 || y == 0 || y == 1)
                    {
                        MakeBlock(y, currentDepth, lifeWeight);
                ...
                else if (totalDepth == 52 || totalDepth == 54)
                {
                    if (y == -2 || y == -1 || y == 0 || y == 1 || y == 2)
                    {
                        MakeBlock(y, currentDepth, lifeWeight);
                    }
                }
                else if (totalDepth == 53)
                {
    ...
    void MakeBlock(int step, DepthBase currentDepth, float lifeWeight)
    {
        ...
        else if (step == -2 || step == 2)
        {
            blockList.Add(
            Instantiate(block1Prefab, new Vector3(0, BlockGap * step, 0), Quaternion.identity)
                .Init(currentDepth.transform, Random.Range(lifeWeight * 2f, lifeWeight * 3f), 3, true));
        }
        else if (step == -1 || step == 1)
        {
            blockList.Add(
            Instantiate(block1Prefab, new Vector3(0, BlockGap * step, 0), Quaternion.identity)
                .Init(currentDepth.transform, Random.Range(lifeWeight * 1.5f, lifeWeight * 2.5f), 2, true));
        }
        else
        {
            blockList.Add(
            Instantiate(block1Prefab, new Vector3(0, BlockGap * step, 0), Quaternion.identity)
                .Init(currentDepth.transform, Random.Range(lifeWeight * 1f, lifeWeight * 2f), 1, true));
        }
    }
}
